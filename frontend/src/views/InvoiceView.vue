  ...
  <v-form @submit.prevent="submitForm">
    ...
    <v-card elevation="4" class="mx-8" rounded="lg">
      ...
      <v-row class="ml-2 mr-2">
        <v-col cols="12">
          <v-slider
            v-model="invoice.depositPercentage"
            :min="0"
            :max="100"
            step="1"
            label="Deposit %"
            thumb-label="always"
            :color="Colours.DodgerBlue"
            show-ticks
          >
            <template #append>
      ...
      <v-table density="compact" class="mb-4">
        ...
        <tbody>
          <tr v-for="(entry, index) in entries" :key="index">
            ...
            <td>R {{ entry.total }}</td>
            <td>
              <v-btn icon :color="Colours.Crimson" variant="plain" @click="deleteEntry(index)">
    ...
    <v-card class="mx-8 mt-4" elevation="4" rounded="lg">
      <v-card-text>
        ...
        <v-row class="mt-6 mr-1 mb-4" justify="end">
          <v-btn :color="Colours.Crimson" class="mr-4" @click.prevent="clearEntries"
            >Clear Entries</v-btn
          >
